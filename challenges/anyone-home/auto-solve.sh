...
LISTING_URLS=(
  "${HTTP_BASE}//${PAYLOAD_ROOT}"
  "${HTTP_BASE}/${PAYLOAD_ROOT}"
)

echo "[*] Fetching payload directory listing..."
if ! LISTING_SOURCE=$(download_with_fallback "${LISTING_FILE}" "${LISTING_URLS[@]}"); then
  echo "[!] Failed to retrieve payload directory listing." >&2
  exit 1
fi
echo "[+] Retrieved payload listing from ${LISTING_SOURCE}"

if ! grep -q "key" "${LISTING_FILE}"; then
  echo "[!] Payload directory listing does not mention key file." >&2
  cat "${LISTING_FILE}" >&2 || true
  exit 1
fi

KEY_URLS=(
  "${HTTP_BASE}//${PAYLOAD_ROOT}/key"
  "${HTTP_BASE}/${PAYLOAD_ROOT}/key"
)
